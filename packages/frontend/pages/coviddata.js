...
//import TODAY from './covidstates';
//import MINUS1DAYS from './covidstates_minus1days';
//import MINUS2DAYS from './covidstates_minus2days';
let minus1DaysMap, minus2DaysMap;
const daysBetween = 3;
...
const getPlaces = async () => {
   ...
   const rawToday = await response.json();
   const todayDate = rawToday[0].dateModified;
   ...
   const MINUS1DAYS = await response1.json();
   ...
   const MINUS2DAYS = await response2.json();

   minus1DaysMap =  Object.assign({}, ...MINUS1DAYS.map(s => ({[s.state]: s})));
   minus2DaysMap =  Object.assign({}, ...MINUS2DAYS.map(s => ({[s.state]: s})));

   //const rawToday = TODAY;
   const normalizedPlaces = normalizePlaces(rawToday);
...
const normalizePlaces = (TODAY) => {
   return TODAY.map((todayPlace, i) => {
	
      // how many per day?
      const perDay = todayPlace[comparisonKey] - minus1DaysMap[todayPlace.state][comparisonKey];
   
      // find doubling duration (days) for yesterday -> today
      const doublingDays = findDoublingDays(1, minus1DaysMap[todayPlace.state][comparisonKey], todayPlace[comparisonKey]);
      ...
      const previousDoublingDays = findDoublingDays(1, minus2DaysMap[todayPlace.state][comparisonKey], minus1DaysMap[todayPlace.state][comparisonKey]);
